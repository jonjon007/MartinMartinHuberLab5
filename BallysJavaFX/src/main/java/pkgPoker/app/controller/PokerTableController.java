...
public class PokerTableController implements Initializable {
	...
	//(getResourceAsStream("/include/img/26.png"), 75, 75, true true);
	...
	private HBox getCardHBox(int iPosition) {
	 ...
	 public void Handle_TableState(Table HubPokerTable) {
		...
		public void Handle_GameState(GamePlay HubPokerGame) {
			...
			for (Player plyr : HubPokerGame.getGamePlayers().values()) {
				for (Card Crd : HubPokerGame.getPlayerHand(plyr).getCardsInHand()) {
					if (plyr.getPlayerID().equals(mainApp.getPlayer().getPlayerID())) {
						if (plyr.getiPlayerPosition() == 1) {
							hboxP1Cards.getChildren().add(BuildImage(Crd.getiCardNbr()));
						}else if (plyr.getiPlayerPosition() == 2) { 
							hboxP2Cards.getChildren().add(BuildImage(Crd.getiCardNbr()));				
						}
					}else {
						if (plyr.getiPlayerPosition() == 1) {
	 ...
	 * public void Handle_GameState(GamePlay HubPokerGame) {
		for(Integer iPos : iPositions){
			ArrayList<iCardImage> iCardImgs = HubPokerGame.GetOrder(iPos);
			// Get cards for each pos
			for(iCardImage iCard : iCardImgs){
				//ImageView iv = new ImageView(getClass().getResourceAsStream("include shtuff"));
